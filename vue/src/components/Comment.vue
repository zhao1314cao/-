        ...
        <div style="flex: 1">
          ...
          <div style="margin-bottom: 10px">
            ...
            <div style="color: #888; font-size: 13px margin-bottom: 10px" ><span style="margin-right: 20px">{{ item.time }}</span>
              ...
              <span style="cursor: pointer; margin-left: 20px" @click="del(item.id)" v-if="item.userId===user.id"><i class="el-icon-delete"></i> 删除</span>
            </div>
            <div v-if="item.showReplyInput">
              <el-input type="textarea" placeholder="请输入回复内容" v-model="item.replyContent"></el-input>
              <div style="text-align: right; margin-top: 5px">
                <el-button type="primary" @click="addReply(item)">回复</el-button>
              </div>
            </div>
          </div>
          <!--这是回复-->
          <div style="display: flex;  grid-gap: 20px; margin-bottom: 20px" v-for="sub in item.children" :key="sub.id">
            ...
            <div style="flex: 1">
              ...
              <div style="color: #888; font-size: 13px; margin-bottom: 10px"><span style="margin-right: 20px">{{ sub.time }}</span>
                ...
                <span style="cursor: pointer; margin-left: 20px" @click="del(sub.id)" v-if="sub.userId===user.id"><i class="el-icon-delete"></i> 删除</span>
              </div>
              <div v-if="sub.showReplyInput">
                <el-input type="textarea" placeholder="请输入回复内容" v-model="sub.replyContent"></el-input>
                <div style="text-align: right; margin-top: 5px">
                  <el-button type="primary" @click="addReply(sub)">回复</el-button>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    addReply(item){
      this.$request.post('/comment/add', { content: item.replyContent, pid: item.id, rootId: item.rootId,fid:this.fid,module: this.module }).then(res => {
        if (res.code === '200') {
          this.$message.success('操作成功')
          item.replyContent = ''
